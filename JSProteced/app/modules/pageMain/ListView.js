...
var ListView = React.createClass({
    ...
    componentDidMount: function () {
        ...
        MainAction.getTemplateList(0,'getChannelList', this.initTemplateList);
    },

    initTemplateList: function (length) {
        PageAction.setPageTotal(length, function (index) {
            MainAction.getTemplateList(index,'getChannelList');
        }.bind(this));
    },
...
console.log(tableArray)
        for (var i = 0, length = tableArray.length; i < length; i++) {
            ...
            var date = c360.utils.unix2human(tableArray[i].time).date + c360.utils.unix2human(tableArray[i].time).time;
            ...
            array.push(<div>
                    <span className="flag-top">
                        <span className="flag-style version"><span
                            className={tableArray[i].versionFlag == 1?"glyphicon glyphicon-info-sign green":"glyphicon glyphicon-info-sign red"}></span>{versionVal}</span>
                    ...
                    <span className="flag-bottom">
                        <span className="flag-style time"><span className="glyphicon glyphicon-info-sign orange"></span>抓取时间:{date}</span>
        ...
        c360.server.jsonpInterface('delChannel', data, function (res) {
            if (res.status == 200) {
                MainAction.showAlert('Success', '删除渠道成功!', true, function () {
                    MainAction.getTemplateList(PageStore.pageObject.currentPageIndex,'getChannelList', this.initTemplateList);
                }.bind(this));
            } else {
